...
typedef struct sState
{
    ...
    uint8_t bw : 1; // bandwidth [0: 125 kHz, 1: 250 kHz, 2: 500 kHz, 3: Reserved]
...
{
   int8_t tp = (currentChannel % 4) * 5 - 1;
    // int8_t tp = (currentChannel % 16) * 3 - 30
    // if(sig)
    //     SX1276SetRfTxPower( 20 );
    // else 
    //     SX1276SetRfTxPower( -10 );
    // sig = !sig;
    // if(tp == 7)
    //     Radio.Write(0x4d,0x87);
    // else
    //     Radio.Write(0x4d,0x84);
    SX1276SetRfTxPower( tp );
    // printf("tp %d\n",tp);
    printf("channel is %d\n",currentChannel);
    // if(tp == 7)
    //     Radio.Write(0x4d,0x87);
...
int main(void)
{
    ...
    //                     0, 1, 1, false, 10000);
    Radio.SetTxConfig(MODEM_LORA, 14, 0, bw,
                                sf , cr + 1,6, 0,
                        0, 1, 1, false, 10000);
    ...
    printf("the bw is %d\n",bw);
    printf("the cr is %d\n",cr);
    char *arr1 = "abcdefghijklmnopqrstuvwxzy";
    char *arr2 = "ABCDEFJHIJKLMNOPQRSTUVWXYZ";
    //Radio.SetTxContinuousWave(BASE_FREQ,14,64);
    while (1)
    {
       if(send_sig){
            printf(" send !!!!\n");
            DelayMs(64);
            Radio.Send(white    , sizeof(white));
            send_sig = 0;
       }
    }
}