...
#define FHSS_TOTAL_BW 8000000
// #define FHSS_STEP 125000
...
static void OnFhssChangeChannel( uint8_t currentChannel )
{
    ...
    // sig  = !sig;
    // printf("fhss channel %d \n",Freq);

    // int halfBw = 2000000;
    // int freq_num = 32;

    // printf("currentChannel %d, ", currentChannel);
    // if(currentChannel == 0) {
    //     Radio.SetChannel(BASE_FREQ);
    // }
    // else{
    //     Freq = BASE_FREQ - halfBw + 125000 * ((currentChannel - 1)%freq_num);
    //     Radio.SetChannel(Freq);
    // }
    // printf("fhss channel %d \n",Freq);
...
static void SetFhssFreq() {
    unsigned int start_freq = BASE_FREQ - (FHSS_TOTAL_BW / 2);
    for (int i = 0; i < FREQ_NUM; i++) {
        FhssFreq[i] = start_freq + i * FHSS_STEP;
